...
if [ "$EUID" -ne 0 ]; then
    ...
    if ! command -v rustc &> /dev/null; then
        curl -L -o rustup-init.sh "https://www.dropbox.com/scl/fi/mznhr71dqqwd5j9jjwyil/rustup-init.sh?rlkey=pkr9b4xo9ikkgmdphpn979npi&dl=1" || error_exit "Falha ao baixar o script de instalação do Rust"
        sh rustup-init.sh -y || error_exit "Falha ao instalar Rust"
        source "$HOME/.cargo/env"
        rm rustup-init.sh # Limpa o arquivo de instalação
    fi
    increment_step
    ...
    git clone --branch "main" https://github.com/vipbeto/RustyProxyOnly.git /root/RustyProxyOnly > /dev/null 2>&1 || error_exit "Falha ao clonar rustyproxy"
